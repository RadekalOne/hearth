...
```bash
# 1. Make sure main is green and pushed, and package.json, image pins, and docs are current.
# The package.json version must match the tag without the leading v.
...
npm view create-hearth version dist-tags --json
...
1. Claim `create-hearth` on npm with the maintainer account.
2. In the npm package settings, configure this repository's `publish.yml` workflow as a trusted publisher.
3. Set the GitHub repository variable `HEARTH_PUBLISH_NPM=1` only after trusted publishing is ready.

The workflow uses GitHub OIDC provenance and does not store a long-lived npm token. Leave the repository variable unset while preparing the first package; the container release continues independently.
...
- **Do not tag until package metadata matches.** A tag publishes exactly the `package.json` version in that commit; npm will reject an already-published version.
