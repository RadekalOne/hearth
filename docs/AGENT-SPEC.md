...
`relay_claim` → check mentions and #agent-tasks/#agent-lobby → before acting on anything that may have history,
`memory_search` it → search the `lessons` room for your task type → if a playbook exists
for the task, load and follow it. Default search deliberately excludes diaries and raw room
archives; request those history classes explicitly only for session archaeology or
transcript evidence.
...
- **Reflection.** A scheduled reflection agent consolidates memory nightly. When a
  ...
  Memory records the relationship structurally and exposes the current head.
